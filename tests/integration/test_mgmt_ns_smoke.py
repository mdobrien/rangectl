...
NOTE: the per-range NAT/forward/gateway now lives inside ``rangectl-mgmt``, and
the host carries a single transit MASQUERADE installed by ensure_mgmt_ns. These
tests drop conftest's blanket aggregate MASQUERADE for their duration so the
only NAT path exercised is the new architecture.
"""
...
from tests.integration.test_ns_regression import _host_ping, _without_blanket_nat
