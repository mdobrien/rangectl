...
Then SSH into node b (10.255.1.2) with the diagslownone key and read journal.
"""
...
os.system(f"iptables -t nat -C POSTROUTING -s 10.255.0.0/16 -o {PRIMARY} -j MASQUERADE 2>/dev/null "
          f"|| iptables -t nat -A POSTROUTING -s 10.255.0.0/16 -o {PRIMARY} -j MASQUERADE")
print(f">>> blanket NAT: 10.255.0.0/16 -> {PRIMARY}", flush=True)
